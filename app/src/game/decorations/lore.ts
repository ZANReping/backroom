...
// v53：原 game/decorations.ts，迁入 game/decorations/ 目录（lore=可查看文案；
// 同目录 containers.ts=可搜索容器；renderer/decorations/ 是纯渲染氛围装饰——
// 贴花与低模道具，不进 m.structures、不可交互；本文件的装饰物是 m.structures 里
// solid=false 的可查看结构）。
...
export const DECOR_VIEWS: Record<string, DecorViewDef> = {
  ...
  // v53b：L3 廊道砖墙的大幅画作——先播报画作描述，再随机展开一页笔记残页（docs.ts L3_NOTES）
  bigpainting: { label: '查看 大幅画作',             sanity: -2 }, // 描述文案按 data.tex 见 engine/interact.ts
  // v51：L3 铁栅栏后的风化希腊女像
