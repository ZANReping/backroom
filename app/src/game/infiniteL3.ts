...
export const L3_VARIANT_LORE: Record<string, string[]> = {
  ...
  dark: [
    ...
    '档案建议：在晦暗区段，笑魇与绝缘体的目击报告显著增多。贴着有灯的一侧走，别在黑暗里停留——这里的黑暗不属于你。',
  ],
  assembly: [
...
export function l3FenceAt(seed: number, k: number, r: number): { y: number; gate: number } | null {
  ...
  return { y, gate: roll >= 0.18 ? h32(seed, 0xf4, k, r) % l3CorrW(seed, k, r) : -1 }
}
...
export function genL3ChunkRaw(def: LevelDef, seed: number, cx: number, cy: number, forceVariant?: string): GenChunk {
  ...
  for (let k = kMin; k <= kMax; k++) {
    ...
    for (let r = rMin; r <= rMax; r++) {
      ...
      if (fence.gate < 0) {
        ...
        pushStruct('bargate', X, fence.y, 1, 1, true, true, { open: 0 }) // 一人宽隧道：栅栏门即整段栅栏
      } else {
        if (fence.gate > 0) pushStruct('barfence', X, fence.y, fence.gate, 1, true)
        pushStruct('bargate', X + fence.gate, fence.y, 1, 1, true, true, { open: 0 })
        if (fence.gate < W - 1) pushStruct('barfence', X + fence.gate + 1, fence.y, W - fence.gate - 1, 1, true)
      }
    }
  }
  ...
  if (ROOM) {
    ...
    const scanEdge = (edge: number) => { // 0北 1南 2西 3东
      ...
      for (const [a, b] of runs) {
        ...
        const gate = h01(seed, 0xdf, cx, cy, edge, a) < 0.5 // 50% 铁栅栏门（监狱工业风）
        ...
        if (gate) { // 铁栅栏门落在门洞第一格（2 宽门洞另一格常开）
    ...
    if (variant === 'assembly') {
      ...
      for (let i = 0, n = rr.int(2, 3); i < n; i++) {
        ...
        if (p) entities.push({ type: rr.weighted(def.entities.map((e) => ({ v: e.type, w: e.w }))), x: p.x + 0.5, y: p.y + 0.5 })
      }
  ...
  if (variant !== 'sanct' && (cx !== 0 || cy !== 0)) {
    ...
    for (let i = 0; i < n; i++) {
      ...
      if (p) entities.push({ type, x: p.x + 0.5, y: p.y + 0.5 })
    }
  }

  // ---- 出口（regionHost 超区域保底：每 RS×RS chunk 区域 1 个，电梯 →L4/L5 各半，嵌墙放置）----
  {
