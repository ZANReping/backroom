...
// - boiler 锅炉：圆柱罐体底 r1.4/顶 r1.3 @2.6m（structures.ts case 'boiler'）→ 盒 ±1.2 贴合罐体
...
export function structColliders(s: Structure, m?: GameMap): ColliderBox[] {
  ...
  switch (s.kind) {
    ...
    case 'boiler': {
      ...
      // 钳制不超出自身 footprint（1×1 据点锅炉不外扩）；罐顶 2.6m 不可站
      const r = Math.min(1.2, s.w / 2, s.h / 2)
      return [{ x0: cx - r, y0: cy - r, x1: cx + r, y1: cy + r, top: 2.6, stand: false }]
    }
    case 'sphboiler':
      // v54：砖石基座 1.7×1.7 + 球罐 r0.85（顶 ~2.2m）→ 盒 ±0.85（钳制于 footprint）；球顶不可站
      return [{ x0: cx - Math.min(0.85, s.w / 2), y0: cy - Math.min(0.85, s.h / 2), x1: cx + Math.min(0.85, s.w / 2), y1: cy + Math.min(0.85, s.h / 2), top: 2.2, stand: false }]
    ...
    case 'sofa': case 'servercase': case 'phonograph': { // v54：双人沙发/塔式服务器机箱（同柜类贴墙碰撞约定）；v55：留声机同（修复模型 flushToWall 贴墙而碰撞盒居中的错位）
      const dims: Record<string, [number, number]> = { // [宽(模型局部 X)，深]
        ...
        vending: [0.95, 0.7], // v54：自动售货机精确碰撞（消除整瓦片空气墙；贴墙位移同渲染层 flushToWall）
        sofa: [1.56, 0.78], servercase: [0.55, 0.6], // v54
        phonograph: [0.64, 0.54], // v55：留声机（柜座+喇叭外缘）
      }
    ...
    case 'redpillar': // v55：红木纹方柱（L5 主厅）——柱身 0.62 见方 + 金柱头/础，全高阻挡（钳制不超出 1×1 占地）
      return [{ x0: cx - 0.43, y0: cy - 0.43, x1: cx + 0.43, y1: cy + 0.43, top: FULL_BLOCK, stand: false }]
    case 'pipes': {
      // v55c（任务5）：管道精确碰撞——消除整瓦空气墙，低管道/贴墙管群让出通行：
      if (s.data?.run) {
        // 贴墙平行粗管群（L2 廊道：净宽 3→2 既定观感不变）：管心贴 hug 侧墙 0.25~0.46——碰撞条贴墙侧 0.28 深
        const hug = Number(s.data.side ?? 0) === 1 ? 1 : -1
        return [{ x0: hug > 0 ? s.x + 0.72 : s.x, y0: s.y, x1: hug > 0 ? s.x + 1 : s.x + 0.28, y1: s.y + 1, top: FULL_BLOCK, stand: false }]
      }
      if (s.data?.cross) {
        // 横穿廊道的大小管道（L2 扭曲段，crawl 蹲伏通道）：按模型管径高度分盒——大管顶 1.18 / 小管顶 0.71
        // （低于头顶的悬空管段不再整瓦阻挡；蹲行可从管侧/管下通过）
        return [
          { x0: s.x, y0: s.y + 0.14, x1: s.x + s.w, y1: s.y + 0.46, top: 1.18, stand: false },
          { x0: s.x, y0: s.y + 0.59, x1: s.x + s.w, y1: s.y + 0.77, top: 0.71, stand: false },
        ]
      }
      // 代墙管排（data.wall）与其余：管排即墙——维持整格全高阻挡
      return [{ x0: s.x, y0: s.y, x1: s.x + s.w, y1: s.y + s.h, top: FULL_BLOCK, stand: false }]
    }
    case 'oddtable': // v55：异形小桌（L5 贝弗莉室）——歪腿大桌面 1.3×1.05 @0.69m 可站上
      return [{ x0: cx - 0.6, y0: cy - 0.48, x1: cx + 0.6, y1: cy + 0.48, top: 0.69, stand: true }]
    case 'furnace': // v55：熔炉（L5 锅炉房）——砖砌炉体 0.9×0.8 全高阻挡（烟道贴顶）
      return [{ x0: cx - 0.45, y0: cy - 0.4, x1: cx + 0.45, y1: cy + 0.4, top: FULL_BLOCK, stand: false }]
    case 'treadmill': // v55：跑步机（L5 健身房）——跑带台 0.62×1.5 @0.25m 可站上
      return [{ x0: cx - 0.31, y0: cy - 0.75, x1: cx + 0.31, y1: cy + 0.75, top: 0.25, stand: true }]
    case 'dumbbellrack': // v55：哑铃架（L5 健身房）——双层架 1.2×0.3 @0.8m 不可站
      return [{ x0: cx - 0.6, y0: cy - 0.15, x1: cx + 0.6, y1: cy + 0.15, top: 0.8, stand: false }]
    case 'spinbike': // v55：动感单车（L5 健身房）——车架 0.4×0.9 @0.5m 可站上
      return [{ x0: cx - 0.2, y0: cy - 0.45, x1: cx + 0.2, y1: cy + 0.45, top: 0.5, stand: true }]
    case 'divingboard': // v54：跳台（L5 泳池）——短柱 + 悬挑跳板：板面 0.5×1.0 @0.85m 可站上
      return [{ x0: cx - 0.25, y0: cy - 0.5, x1: cx + 0.25, y1: cy + 0.5, top: 0.85, stand: true }]
    case 'gymbench': // v54：健身卧推凳（L5 健身房）——凳面 0.45×1.1 @0.45m 可站上（杠铃架细柱不另计）
      return [{ x0: cx - 0.23, y0: cy - 0.55, x1: cx + 0.23, y1: cy + 0.55, top: 0.45, stand: true }]
    case 'stairrail': {
      // v54：oldstairs 井口护栏碰撞（仅碰撞无模型；可见护栏在出口模型）——沿走向两侧细条 + 尽头横栏（data.end）
      // data.deg=出口朝向（同 orientStairs 的 rotation.y 角度制：90=墙在 +x/走向 -x；270=墙 -x；0=墙 +y；180=墙 -y）
      const deg = (((Number(s.data?.deg) || 0) % 360) + 360) % 360
      const T = 0.06 // 半厚
      const out: ColliderBox[] = []
      if (deg === 0 || deg === 180) { // 走向沿 y：两侧栏杆在 x=cx±0.55
        out.push({ x0: cx + 0.55 - T, y0: s.y, x1: cx + 0.55 + T, y1: s.y + 1, top: FULL_BLOCK, stand: false })
        out.push({ x0: cx - 0.55 - T, y0: s.y, x1: cx - 0.55 + T, y1: s.y + 1, top: FULL_BLOCK, stand: false })
        if (s.data?.end) { // 尽头横栏：deg 0=走向 -y（北缘）；deg 180=走向 +y（南缘）
          const ey = deg === 0 ? s.y : s.y + 1
          out.push({ x0: s.x, y0: ey - T, x1: s.x + 1, y1: ey + T, top: FULL_BLOCK, stand: false })
        }
      } else { // 走向沿 x：两侧栏杆在 y=cy±0.55
        out.push({ x0: s.x, y0: cy + 0.55 - T, x1: s.x + 1, y1: cy + 0.55 + T, top: FULL_BLOCK, stand: false })
        out.push({ x0: s.x, y0: cy - 0.55 - T, x1: s.x + 1, y1: cy - 0.55 + T, top: FULL_BLOCK, stand: false })
        if (s.data?.end) { // 尽头横栏：deg 90=走向 -x（西缘）；deg 270=走向 +x（东缘）
          const ex = deg === 90 ? s.x : s.x + 1
          out.push({ x0: ex - T, y0: s.y, x1: ex + T, y1: s.y + 1, top: FULL_BLOCK, stand: false })
        }
      }
      return out
    }
    case 'glasswin': {
      // v54b：L4 雨窗（data.deg 定向变体）精确碰撞——窗体是贴瓦片外缘的薄墙板（框深 0.22 + 玻璃全高不可穿），
      // 碰撞条贴同侧瓦片缘（消除整瓦空气墙）；无 deg 变体（L274 彩窗等贴墙件）保持整瓦默认
      if (s.data?.deg === undefined) return [{ x0: s.x, y0: s.y, x1: s.x + s.w, y1: s.y + s.h, top: FULL_BLOCK, stand: false }] // 非定向变体：整瓦默认
      const deg = (((Number(s.data.deg) || 0) % 360) + 360) % 360
      const T = 0.26 // 碰撞条厚（覆盖框 0.28..0.50 并略内收）
      if (deg === 0) return [{ x0: s.x, y0: s.y + s.h - T, x1: s.x + s.w, y1: s.y + s.h, top: FULL_BLOCK, stand: false }]
      if (deg === 180) return [{ x0: s.x, y0: s.y, x1: s.x + s.w, y1: s.y + T, top: FULL_BLOCK, stand: false }]
      if (deg === 90) return [{ x0: s.x + s.w - T, y0: s.y, x1: s.x + s.w, y1: s.y + s.h, top: FULL_BLOCK, stand: false }]
      return [{ x0: s.x, y0: s.y, x1: s.x + T, y1: s.y + s.h, top: FULL_BLOCK, stand: false }]
    }
    case 'tvset': {
      // v54：立式大电视——机身 1.06×0.5（含底座），deg 90/270 换轴
      const deg = (((Number(s.data?.deg) || 0) % 360) + 360) % 360
      const hw = 0.53, hd = 0.25
...
// v54：三层（floors>2）时与第三层天花拉平（≥2×FLOOR_H+2.6）
export function tallCeilH(m: GameMap, wallH: number): number {
  const t = wallH * 1.75
  if (m.floors > 2) return Math.max(t, 2 * FLOOR_H + 2.6)
...
export function wallBaseTopAt(m: GameMap, x: number, y: number, wallH: number): { base: number; top: number } | null {
  ...
  // v54c 多层解耦：正上方压着上层楼板/上层墙（本格 up/up2/upWall）时，本墙止于楼板底——
  // 不再被邻格的上层楼板牵着升高（旧规则让 1F 间墙穿过上层地板，上层平面被 1F 轮廓牵死）
  const i0 = y * m.w + x
  if (m.up[i0] === 1 || m.upWall[i0] === 1) top = Math.min(top, FLOOR_H - 0.35)
  else if (m.up2[i0] === 1 || m.upWall2[i0] === 1) top = Math.min(top, 2 * FLOOR_H - 0.35)
  return { base, top }
}
...
export function ceilingSteps(m: GameMap, wallH: number): CeilStep[] {
  ...
  for (let y = 0; y < m.h; y++)
    for (let x = 0; x < m.w; x++) {
      const i = y * m.w + x
      if (m.tiles[i] !== 1 || m.outdoor[i] === 1 || m.up[i] === 1 || m.up2[i] === 1 || m.crawl[i] === 1 || m.ceiling[i] === 1) continue
      for (const [dx, dy, d] of [[0, -1, 0], [1, 0, 1], [0, 1, 2], [-1, 0, 3]] as const) {
        const nx = x + dx, ny = y + dy
        if (nx < 0 || ny < 0 || nx >= m.w || ny >= m.h) continue
        const ni = ny * m.w + nx
        if (m.tiles[ni] !== 1 || m.outdoor[ni] === 1 || m.up[ni] === 1 || m.crawl[ni] === 1) continue
        if (m.ceiling[ni] === 1) { out.push({ x, y, dir: d, lo: wallH, hi: tallH }); continue }
        // v54e：邻格为「仅 3F 板」挑空（up=0 且 up2=1，如 Gamma 前厅中庭）——低顶（wallH）到
        // 3F 板底（2×FLOOR_H−0.35）之间同样要填薄墙封边（门洞/窗格低顶格与挑空中庭直接相邻时的镂空黑洞）
        if (m.up2[ni] === 1) out.push({ x, y, dir: d, lo: wallH, hi: 2 * FLOOR_H - 0.35 })
      }
    }
  return out
}
...
// 蹲伏风道 = 风道底 1.15m；上层走 band=1/2 的对应层天花板（v54：band2=三层天花 2×FLOOR_H+2.6，
...
export function ceilingHeightAt(m: GameMap, x: number, y: number, wallH: number, band: FloorBand = 0): number {
  ...
  if (band === 1) {
    ...
    if ((m.stair[i] & 7) !== 0 && m.up2[i] === 1) return 2 * FLOOR_H + 2.6 // v54：2F→3F 坡道上空（up2=可站标记而非楼板）
    return m.up2[i] === 1 ? 2 * FLOOR_H - 0.35 : FLOOR_H + 2.6 // v54：上方有三层楼板时=其板底
  }
...
function genOnce(def: LevelDef, seed: number): GameMap {
  ...
  const m: GameMap = {
    ...
    up2: new Uint8Array(size * size), // v54：三层楼板（缺省全 0=无三层）
  ...
  switch (def.gen) {
    ...
    case 'hotel': {
      ...
      // v54：楼梯间装饰梯删除（纯装饰不可交互——全游戏只保留 data.climb 攀爬梯）
  ...
  // v54：装饰梯（不可交互）全部删除——uniCount 不再含 ladder；攀爬梯走 addClimbLadder（data.climb）
  ...
  // v54e：主层 2D BFS 只看主层（floor=0）结构——上层家具（Gamma 2F 书架/沙发压在 1F 食堂上空）
  // 曾参与主层连通判定，把 1F 地板围成「不可达孤岛」被回填成墙（墙洞+灯下不来）
  const openableAt = (x: number, y: number) =>
    m.structures.some((s) => s.solid && (s.floor ?? 0) === 0 && OPENABLE.includes(s.kind) && x >= s.x && x < s.x + s.w && y >= s.y && y < s.y + s.h)
  const passFloor = (x: number, y: number): boolean => {
    if (x < 0 || y < 0 || x >= size || y >= size) return false
    if (m.tiles[y * size + x] !== 1) return false
    return !solidStructAtFloor(m, x, y, 0) || openableAt(x, y)
  }
  ...
  for (let i = 0; i < size * size; i++) {
    // v54：楼梯坡道格不回填——2F→3F 坡道（base=FLOOR_H）的瓦片在 2D 主层 BFS 里必然不可达
    //（坡道高度 >JUMP_REACH），回填会把坡道填成墙、毁掉三层楼梯间（Gamma 基地）
    if (m.tiles[i] === 1 && !reach[i] && m.outdoor[i] === 0 && !structMark[i] && m.stair[i] === 0) m.tiles[i] = 2
  }
  ...
  // v55（任务8）：床类床头靠墙——bed/bunkbed/hospitalbed 未显式 data.deg 的，
  // 按床格短端邻墙自动补朝向（楼层带感知：1F/2F 床查对应 upWall/upWall2）
  for (const s of m.structures) {
    if (s.kind !== 'bed' && s.kind !== 'bunkbed' && s.kind !== 'hospitalbed') continue
    if (s.data?.deg !== undefined) continue
    const wa = s.floor === 1 ? m.upWall : s.floor === 2 ? m.upWall2 : null
    const isWall = (tx: number, ty: number) =>
      tx < 0 || ty < 0 || tx >= m.w || ty >= m.h ? false : wa ? wa[ty * m.w + tx] === 1 : m.tiles[ty * m.w + tx] !== 1
    const d = bedHeadDeg(isWall, s.x, s.y, s.w, s.h)
    if (d !== null) s.data = { ...s.data, deg: d }
  }
  ...
  // 出口光源
  // v54：设计模式重制的据点可能已把出口灯拖到自定义位置（applyDesignPatch 移动出口后，
  // 玩家拖过的出口灯随 lightAdd 落地）——出口 1.5 格内已有同色出口灯时跳过自动补灯，避免重复
  for (const e of m.exits) {
    if (m.lights.some((l) => l.color === '#f5e37a' && Math.hypot(l.x - (e.x + 0.5), l.y - (e.y + 0.5)) < 1.5)) continue
    m.lights.push({ x: e.x + 0.5, y: e.y + 0.5, r: 3, color: '#f5e37a', flickerSeed: rng.next() * 100 })
  }
  // 壁灯/灯阵附加光
  for (const s of m.structures) {
...
// 铺设直线楼梯跑道：从 (x,y) 起沿 dir 铺 n 格，从 base 爬到 base+FLOOR_H（每格升 FLOOR_H/n，≤0.65 可直接走上）
// 返回顶端 landing 格坐标（跑道尽头前一格，调用方保证其上一层楼板=1）。
// 跑道格 hi 越过对应带中线的标记该层可站（base=0→up，base=FLOOR_H→up2[v54]：2F→3F 坡道）。
// v54：base 参数支持 2F→3F 坡道（坡道从 2F 楼板起坡：全程 up=1 保留，up2 仅上段标记）
export function stampStairRun(m: GameMap, x: number, y: number, dir: 1 | 2 | 3 | 4, n: number, base = 0): { lx: number; ly: number } {
  ...
  for (let k = 0; k < n; k++) {
    ...
    m.tiles[ii] = 1
    m.stair[ii] = encStair(dir, Math.round(base * 100) + stepCm * k, Math.round(base * 100) + stepCm * (k + 1))
    m.step[ii] = 0; m.crawl[ii] = 0; m.elev[ii] = 0
    if (base === 0) {
      if (stepCm * (k + 1) / 100 > BAND_MID) m.up[ii] = 1 // 坡道上半段：上层可行走
      else m.up[ii] = 0
    } else {
      // 2F→3F 坡道（v54）：可站标记按坡道面实际高度带分段——下段（lo<4.5）2F 可站、上段（hi>4.5）3F 可站，
      // 交界格两带均可站（换带连续）；上段不再标记 up=1（其表面已在 3F 高度带，2F 走不上去）
      if (base + stepCm * k / 100 < FLOOR_H + BAND_MID) m.up[ii] = 1
      else m.up[ii] = 0
      if (base + stepCm * (k + 1) / 100 > FLOOR_H + BAND_MID) m.up2[ii] = 1
      else m.up2[ii] = 0
    }
  }
...
// ---------- 跨层连通 BFS（状态 = 瓦片 × 楼层带）----------
// band0：主层地板（含楼梯坡道）；band1：上层楼板（up=1 且非 upWall）；band2：三层楼板（up2=1 且非 upWall2，v54）。
// 同层四向通行遵守 JUMP_REACH 高差规则；楼梯格可在坡道到达的楼层带间自由切换（连续坡道）；
// lift 格 0↔1 两层互通（电梯交互）；climb 梯子 base↔top 直连。
// v54：步长 S = 楼层数（三层地图 reach 下标 i*3+b；双层保持 i*2+b——EL3A 既有断言不受影响）
export function bfs3D(m: GameMap): Uint8Array {
  const W = m.w, H = m.h
  const S = (m.floors ?? 1) >= 3 ? 3 : 2 // v54：状态步长
  const reach = new Uint8Array(W * H * S)
  const OPENABLE: readonly string[] = ['hoteldoor', 'rollerdoor', 'glassdoor', 'bargate'] // v51：bargate 栅栏门
  const openable = (x: number, y: number, floor: 0 | 1 | 2) =>
    m.structures.some((s) => s.solid && OPENABLE.includes(s.kind) && (s.floor ?? 0) === floor && x >= s.x && x < s.x + s.w && y >= s.y && y < s.y + s.h)
  // 行走高度（null=该层带不可站立）
  const walkH = (x: number, y: number, band: 0 | 1 | 2): number | null => {
    if (x < 0 || y < 0 || x >= W || y >= H) return null
    const i = y * W + x
    const s2 = m.stair[i]
    if (s2 & 7) {
      // v54：上层带走坡道需「坡道服务该带 + 该层楼板可站标记」；主层带不限（高差规则自然拦截悬梯段）
      if (band >= 1 && (!stairServesBand(s2, band, JUMP_REACH) || upAt(m, band as 1 | 2)[i] !== 1)) return null
    ...
    if (band === 2) { // v54：三层楼板
      if (m.up2[i] !== 1 || m.upWall2[i] === 1) return null
      if (solidStructAtFloor(m, x, y, 2) && !openable(x, y, 2)) return null
      return 2 * FLOOR_H
    }
  ...
  const q: [number, number, 0 | 1 | 2][] = [[m.spawn.x, m.spawn.y, 0]]
  reach[(m.spawn.y * W + m.spawn.x) * S] = 1
  while (q.length) {
    const [x, y, b] = q.pop()!
    const h0 = walkH(x, y, b)
    if (h0 === null) continue
    const push = (nx: number, ny: number, nb: 0 | 1 | 2) => {
      const ii = (ny * W + nx) * S + nb
      if (reach[ii]) return
    ...
    const link = (nx: number, ny: number, nb: 0 | 1 | 2) => {
      const ii = (ny * W + nx) * S + nb
      if (reach[ii] || walkH(nx, ny, nb) === null) return
      reach[ii] = 1; q.push([nx, ny, nb])
    }
    for (const [dx, dy] of [[1, 0], [-1, 0], [0, 1], [0, -1]] as const) push(x + dx, y + dy, b)
    // 楼梯格：坡道到达的楼层带间自由切换（v54：三段坡道可切 1↔2，一段坡道 0↔1，不跨段）
    if (m.stair[y * W + x] & 7) for (const nb of [0, 1, 2] as const) if (nb !== b) link(x, y, nb)
    // 电梯：同格两层互通
