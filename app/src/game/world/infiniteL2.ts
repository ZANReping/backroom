...
import { registerInfiniteLevel, bedHeadDeg, type GenChunk } from './infiniteRegistry'
import { jerryFollowerDef, type NpcDef } from '../content/npcs' // v45：信众宣传间驻防 NPC
            ...
            const placeFurn = (kind: Structure['kind'], w: number, h: number, solid: boolean, withSid = false, data?: Structure['data']): boolean => {
              ...
              for (const [i, j] of cand)
                if (furnOK(i, j, w, h)) {
                  ...
                  // v55（任务8）：床类床头靠墙——短端邻墙自动补 data.deg（房间矩形外即墙）
                  let d2 = data
                  if ((kind === 'bed' || kind === 'bunkbed' || kind === 'hospitalbed') && d2?.deg === undefined) {
                    const bd = bedHeadDeg((tx, ty) => !(tx >= x0 && tx <= x1 && ty >= y0 && ty < y0 + rh), i, j, w, h)
                    if (bd !== null) d2 = { ...d2, deg: bd }
                  }
                  pushStruct(kind, i, j, w, h, solid, withSid, d2)
                  return true
                }
              return false
            }
