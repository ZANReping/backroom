...
export const DEEP_ENTITIES: Record<string, EntityDef> = {
  ...
  tiny: {
    type: 'tiny', name: '小小', hp: 90, speed: 3.4, damage: 24, sight: 9, hearing: 16,
    hearsSprint: true, color: '#5a7a86', habitat: 'outdoor', aquatic: true, // v58：极快泳速 + 远距听觉
    desc: '巨大的类人形实体。焦油般的外壳之下是橡胶质的坚韧皮肤与两栖动物般的荧光斑点；面覆甲壳，只露出两只眼睛。',
    codex: {
      no: 'Entity 720「Tiny」', danger: '4 级（高威胁）', habitat: 'Level 7 · 暮色带环形场',
      behavior: '智力极高且健谈，但傲慢、恶意毫不掩饰；厌恶噪音与肢体接触，被激怒后拒绝一切对话。泳速极快，能在极远处捕捉猎物的动静；无法离开水体。面部甲壳张开=追击的最终阶段。',
      counter: '安静下潜、避免巨响，它愿意交谈时不要提「出口」。被激怒后没有任何谈判余地——它的长矛由某种巨物的尸骨制成。',
      lore: [
        '首次记录于 2019 年 4 月 18 日。名字来自最初那份报告里的一句自嘲，之后就没有人再笑得出来。',
        '小小的身体大部分被厚厚的焦油包裹，可随意释放焦油迷惑或迟滞猎物；焦油层之下的皮肤点缀着生物荧光斑点，近似两栖动物，橡胶质感却坚韧如天然盔甲。',
        '它的眼睛在光照减弱时会变得明亮，浮出水面时却几乎暗淡无光。据流浪者报告，当面部甲壳打开、露出尖牙密布的巨口时，便是它追击的最终阶段。',
        '⚠ Wikidot 的 Level 7 条目已被官方标记为 outdated。本档案形象参照 backrooms-wiki-cn 的 Entity 720 条目重建。',
      ],
      sighting: '「它问我们是不是又来了一位客人。我说是。它笑了，说这里没有什么门。——可它身后的石台上明明嵌着一扇。」——无名流浪者，未归档录音。',
    },
    aggroStinger: true,
  },
  thething: {
  ...
  wretch: {
    ...
    codex: {
      ...
      behavior: '行为近似僵尸：迟缓、执着、成群。它们曾经是人——转化过程被认为源于食物、水与睡眠的长期剥夺。Level 3 有极其罕见的目击记录。',
