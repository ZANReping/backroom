...
with sync_playwright() as pw:
    ...
    def hop(wx, wy):
        for _ in range(80):
            cur = pg.evaluate('() => [window.__engine.player.x, window.__engine.player.y]')
            dx, dy = wx - cur[0], wy - cur[1]
            if math.hypot(dx, dy) < 2:
                return True
            step = min(24, math.hypot(dx, dy))
            nx, ny = cur[0] + dx / math.hypot(dx, dy) * step, cur[1] + dy / math.hypot(dx, dy) * step
            pg.evaluate(f'() => {{ const p = window.__engine.player; p.x = {nx}; p.y = {ny}; p.z = 0; p.vz = 0 }}')
            pg.wait_for_timeout(450)
        return False

    def shoot(key, wx, wy, dist, pitch):
        if not hop(wx, wy):
            print(key, 'HOP FAIL'); return
        r = pg.evaluate(f'''() => {{
