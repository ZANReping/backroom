# B.N.T.G. 存储设施（id 107，v54 据点）贴图下载与处理（ambientCG CC0，流程同 SOURCES.md 既有条目）：
# 下载 1K JPG → 居中裁方 → 512×512 → 去色至近中性灰 → 亮度归一 → JPEG q86
# 素材选择（全新下载，不复用既有素材；占用清单见 SOURCES.md）：
#   墙=CorrugatedSteel003（波纹钢——仓库工业墙，与 EL3A 的 CorrugatedSteel009 不同款）；
#   地=Concrete028（仓库混凝土）；顶=PaintedPlaster016（涂装粉刷吊顶——OfficeCeiling 系列均被占）
# 用法：cd app && python scripts/gen-l107-textures.py（本机 TLS 需 curl --tlsv1.2 -k 先入库缓存）
import io
import os
import zipfile
...
JOBS = {
    'l107_wall.jpg': ('CorrugatedSteel003', 0.72, 0),  # 波纹钢工业墙（BNTG 灰绿乘色见 l107.ts palette）
    'l107_floor.jpg': ('Concrete028', 0.66, 0),        # 仓库混凝土地面
    'l107_ceil.jpg': ('PaintedPlaster016', 0.80, 0),   # 涂装粉刷吊顶（办公角天花）
}


def fetch_color_jpg(asset_id: str) -> Image.Image:
    # 优先本地缓存（scripts/.cache-l107/<ID>.zip——本机 TLS 被截断，
    # 先用 `curl --tlsv1.2 -k -L -o scripts/.cache-l107/<ID>.zip https://ambientcg.com/get?file=<ID>_1K-JPG.zip` 入库）；
    # 无缓存时直接联网下载（下载地址见 SOURCES.md 末尾）
    cache = f'scripts/.cache-l107/{asset_id}.zip'
