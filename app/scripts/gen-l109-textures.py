# M.E.G. Omega 基地（id 109，v54 据点）贴图下载与处理（ambientCG CC0，流程同 SOURCES.md 既有条目）：
# 下载 1K JPG → 居中裁方 → 512×512 → 去色至近中性灰 → 亮度归一 → JPEG q86
# 素材选择（全新下载，占用清单见 SOURCES.md）：
#   墙=PaintedPlaster004（v54c 再换：平整净白粉刷——现代办公室风；003/008/013/015/016/017 均占用或弃用）；
#   地=Carpet014（整洁浅灰方块地毯——现代办公；002/007/013 均占用或弃用）；顶=PaintedPlaster010（粉刷吊顶——OfficeCeiling 系列均被占）
# 用法：cd app && python scripts/gen-l109-textures.py（本机 TLS 需 curl --tlsv1.2 -k 先入库缓存）
import io
import os
import zipfile
...
JOBS = {
    'l109_wall.jpg': ('PaintedPlaster004', 0.86, 0),  # 平整净白粉刷墙（现代办公室风）
    'l109_floor.jpg': ('Carpet014', 0.72, 0),         # 整洁浅灰方块地毯
    'l109_ceil.jpg': ('PaintedPlaster010', 0.84, 0),  # 粉刷吊顶
}


def fetch_color_jpg(asset_id: str) -> Image.Image:
    # 优先本地缓存（scripts/.cache-l109/<ID>.zip——本机 TLS 被截断，
    # 先用 `curl --tlsv1.2 -k -L -o scripts/.cache-l109/<ID>.zip https://ambientcg.com/get?file=<ID>_1K-JPG.zip` 入库）；
    # 无缓存时直接联网下载（下载地址见 SOURCES.md 末尾）
    cache = f'scripts/.cache-l109/{asset_id}.zip'
