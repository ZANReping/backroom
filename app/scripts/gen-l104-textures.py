# Tom 的餐馆（独立餐馆据点）贴图下载与处理（ambientCG CC0，流程同 SOURCES.md 既有条目）：
# 下载 1K JPG → 居中裁方 → 512×512 → 去色至近中性灰 → 亮度归一 → JPEG q86
# 用法：cd app && python scripts/gen-l104-textures.py
import io
import os
import zipfile
...
JOBS = {
    'l104_wall.jpg': ('Plaster005', 0.80, 0),        # 餐馆墙面（暖色粉刷，乘色见 ltom palette）
    'l104_floor.jpg': ('WoodFloor043', 0.62, 0),     # 餐馆地面（暖色木地板）
    'l104_ceil.jpg': ('OfficeCeiling001', 0.82, 0),  # 餐馆吊顶（办公吊顶）
}


def fetch_color_jpg(asset_id: str) -> Image.Image:
    # 优先本地缓存（scripts/.cache-l104/<ID>.zip，urllib 的 TLS 在部分网络下不通时先用 curl 下载）；
    # 无缓存时直接联网下载（下载地址见 SOURCES.md 末尾）
    cache = f'scripts/.cache-l104/{asset_id}.zip'
