...
> 基于当前代码库（v32）整理。本文档描述系统架构、层级/物品/现象内容与开发者工具。
...
└── game/
    ...
    ├── entities/             # 31 种实体定义与 AI + spawns.ts（层级/威胁/稀有度派生表）
    ├── phenomena.ts          # 现象注册表（孤立效应/植殖癌/闪烁）
    ├── docs.ts               # M.E.G. 文档注册表
    ├── items.ts              # 66 件物品定义 + 稀有度/后室物品标记
...
- 近战：武器伤害（拳 8 / 木板 22 / 撬棍 25 / 刀 30 / 扳手 20 / 斧 45），
  ...
  尸体不会被玩家一锤钉进墙里
- 29+2 种实体 AI：游荡/调查/追击/攻击/埋伏/伪装；听觉（冲刺噪音）、视线、栖息地过滤；
...
| mesh-smoke.mts | 32 实体 + 68 物品（含**零 fallback 断言**，v40）+ 129 结构（v43 新增 pallet/handrail/machinewall；v45 新增 domering/perch 穹顶结构件与栖木 + 鹉主杰瑞蓝色鹦鹉模型；v46 新增 walllamp 壁挂斜照灯；v47 新增 pulpit/candlestand/holyfont 教堂件）+ 玩家模型（576 组形象配置 + **女性体型节点断言**）+ **NPC 配饰全量附加**（v40）建模 + **v48 朝向断言**（贴墙场景：柜类/转椅背贴最近墙正面朝室内、邻桌朝桌优先、data.deg 覆盖、glasswin/蓝彩玻贴墙面朝室内） |
