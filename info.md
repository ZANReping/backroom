...
└── game/
    ...
    │                         #   renderer3d.ts 兼容层（仅 re-export ../renderer）/ audio.ts / rng.ts / storage.ts /
    │                         #   keybinds.ts / avatar.ts / llm.ts 可选 LLM API（OpenAI 兼容；
    │                         #   NPC 自由对话，失败回退预制）
    ├── world/                # 地图/世界生成：
    │                         #   mapgen.ts GameMap + 有限层生成 + 碰撞盒/高度/容器表 /
    │                         #   mapgenDeep.ts v23：L6–L11 与 L601 地形生成器（对 mapgen 仅 type-only 引用）/
    ...
    │                         #   铁栅栏/栅栏门 + 双灯光变体）/ infiniteRegistry.ts 无限层级注册表（避免循环依赖）
    ├── content/              # 内容/数据注册：
    │                         #   phenomena.ts 现象注册表（孤立效应/植殖癌/闪烁）/ docs.ts M.E.G. 文档注册表 /
    │                         #   items.ts 80 件物品定义 + 稀有度/后室物品标记 /
...
### 3.1 无限 chunk 生成（L0/L1/L2/L3/L4）
...
- **共享边哈希缝合**：L0/L1 用 `edgeOpen(seed, vertical, a, b)` 两侧 chunk 对同一条边算出同一
  ...
  **v54 可选 outdoor 室外瓦片数组**——L4 窗景区窗外虚空，其余层缺省全室内）。
...
### 3.3e Level 4「废弃办公室」无限化重制（v54，`infiniteL4.ts`）
...
### 3.3f Level 5「恐怖酒店」无限化重制（v54，`infiniteL5.ts`）
...
**v55 走廊精致化**：走廊地面 tint 21 红金华丽地毯（geometry carpetGeos 独立网格、世界 UV 3m 团花周期）+
...
data.tex/data.layer；跨 chunk 统一 pushClipped 裁剪推送）——走廊 runner 竖廊 3×8/横廊 8×2 首尾相接 + 大厅/房间
整齐地毯块；贴图 l5_carpet/l5_carpet_blue/l5_tile（scripts/gen-l5-textures.py PIL 可复现，SOURCES.md 登记）。
...
### 7.9 v51 之后版本纪要（v52~v55）

按主题归并，每条一句话；细节见对应章节（§3.3d/§3.3e/§3.8/§3.9/§5 等）。
...
- Level 4 无限化重制（走廊网+四区段、雨声、古典楼梯/活板门/嵌墙电梯出口链；v54b 假楼梯已删）——见 §3.3e。
...
- Level 5 重制为无限生成 1930 年代酒店综合楼（infiniteL5.ts，CS=32 纯函数、世界坐标哈希缝合）：
  ...
  主厅挑高（GenChunk/LiveChunk 新增可选 ceiling/liquid 数组经 stitch 缝合——无限层首例）——详见 §3.3f。
...
- 走廊精致化：红金团花地毯贴图（tint 21 独立网格）+奶白墙裙+金腰线+整齐灯带；房间分色 tint ×5；
  踢脚线加 id 5；rug 地毯结构（廊内 runner 首尾相接 + 厅房整齐地毯块，pushClipped 跨 chunk 裁剪）。
...
- 新结构：rug/redpillar/ceilingbeam/darkdoorblock（168 种建模全过）；新贴图 l5_carpet/l5_carpet_blue/l5_tile（PIL 可复现）。
